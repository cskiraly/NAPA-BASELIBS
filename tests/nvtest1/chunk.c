...
void chbuf_enqueue_chunk(struct chunk *ch) {
	if(chbuf_get(ch->chunk_num, NULL, NULL) == CB_NOT_YET_HERE) {
		struct chunk_env *newchunk = (struct chunk_env *)malloc(sizeof(struct chunk_env) + ch->len);
	    memcpy(&newchunk->chunk,ch,sizeof(struct chunk) + ch->len);
        chbuf_enqueue(newchunk);
	}
}
...
int chbuf_get(int chunk_num, unsigned char **chunk_data, struct timeval *chunk_time) {
	...
	else if(currchunk->chunk.chunk_num != chunk_num) return currchunk->chunk.chunk_num > chunk_num ? CB_OVERWRITTEN : CB_NOT_YET_HERE;
